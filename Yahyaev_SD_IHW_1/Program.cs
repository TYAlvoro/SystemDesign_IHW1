...
using Yahyaev_SD_IHW_1.DataExport;
using Yahyaev_SD_IHW_1.DataImport;
...
namespace Yahyaev_SD_IHW_1
{
    class Program
    {
        ...
        private static void CreateTestData()
        {
            ...
            var account = _facade.CreateBankAccount("Основной счет");
            var categoryIncome = _facade.CreateCategory(Category.CategoryType.Income, "Зарплата");
            var categoryExpense = _facade.CreateCategory(Category.CategoryType.Expense, "Кафе");
            _facade.CreateOperation(Operation.OperationType.Income, account, 1000m, DateTime.Now.AddDays(-2), "Зарплата за февраль", categoryIncome);
            _facade.CreateOperation(Operation.OperationType.Expense, account, 200m, DateTime.Now.AddDays(-1), "Обед в кафе", categoryExpense);
        }

        private static void TestJSONExport()
        {
            _jsonExporter.Export(_facade, "export_data.json");
            Console.WriteLine("Экспорт данных в JSON завершен. Файл: export_data.json");
        }
        ...
        static void Main(string[] args)
        {
            ...
            // Интерфейсное меню для взаимодействия с пользователем
            bool exit = false;
            while (!exit)
            {
                Console.WriteLine("\n--- Меню ---");
